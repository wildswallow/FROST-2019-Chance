...
ggplot(data=mergedfsort,aes(x=factor(question),y=Avg,colour=dsign)) + scale_x_discrete(breaks = 1:47) +
  ...
  facet_grid(.~Category2, scale="free_x", space="fixed", labeller = labeller(Category2 = label_wrap_gen(10))) + 
...
if (length(levels(data$textbook))==3) {
  ...
  data_box$InstructorName_Section_Semester <- gsub("Section.*$","",data_box$InstructorName_Section_Semester) #Add if want to do by instructor (removes Section + Semester for unit labels), data formatted differently in 1617
  means <- tapply(data_box[,my_rv],data_box$textbook,mean)
  grand_mean <- mean(data_box[,my_rv])
  dev.off()
  ggplot(aes_string(y=my_rv,x="textbook",group="InstructorName_Section_Semester",fill="textbook"),data=data_box) + geom_boxplot() + xlab("Section") + ylab(my_rv_lab) + ggtitle(paste("Boxplots of", my_rv_lab,"by Instructor and Textbook")) + theme_bw() +
    geom_segment(aes(x=.647,y=means[1],xend=1.355,yend=means[1]), colour="deeppink4", size=1.3) +
    geom_segment(aes(x=1.613,y=means[2],xend=2.355,yend=means[2]), colour="deeppink4", size=1.3) +
