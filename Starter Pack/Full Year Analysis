...
#######################################################################################  For HS students, check first section of code
############################## Auto-Cleaning ##########################################  
#######################################################################################  
...
### Any dataset involving 1617 won't look nice due to NA's; use others
...
my_rv = data$new_post_perc
# data$textbook = factor(data$textbook, levels = c("ISI", "OtherSBI", "NotSBI"))  #running this competely changes the textbook vars. and sample sizes
...
###all data=data_box for 1617, running everything above
...
nullmodel <- lmer(new_ach_gain ~  ( 1 | pre_perc), data=data_box, REML = FALSE)
summary(nullmodel)
...
#count n per year
  table(data_box$year)
#checking (NUMERIC) vars (one at a time) for how it looks
  var = "stats_career_usage_post"
  ggplot(aes_string(y = var, x="year",group="year",fill="year"),data=data_box) + geom_boxplot() + xlab("Year") + ggtitle(paste("Boxplots of", var,"by Year")) + theme_bw() 
#for CATEGORICAL vars 
  table(data_box$new_ach_gain, data_box$year)
#finds NA's by variable by year
  aggregate(incentive.pre ~ year, data=data_box, function(x) {sum(is.na(x))}, na.action = NULL)
# % of NA's
  sum(is.na(data_box$mastering_confidence_post))/nrow(data)
#View specific columns together
  View(subset(data_box, select = c("incentive.pre","math_grades_post")))
#Check all vars in cleaned data=allyears for NA's, crosscheck with excel variables explained
  View(apply(data_box, MARGIN = 2, function(x) sum(is.na(x))))
...
View(full_col_names)
library(openxlsx)
#write.xlsx(full_col_names, "Same Column Names across all 3 Years.xlsx")

#(incomplete)
#table of #sections per instructor per term per year
#can use instructor.last.name, year, section (kinda), semester (kinda)
#recode the 7 levels of semester (incl. Fullyear) into just Fall and Spring
data$semester_fs <- NA
data$semester_fs[data$semester=="fall" | data$semester=="Fall"] <- "Fall"
data$semester_fs[data$semester=="spring"| data$semester=="Spring"] <- "Spring"
data$total_num_section <- NA
#need to record max section number, and put that as a replacement to instructor.last.name
install.packages("expss")
View(cro(data$instructor.last.name, list(total(), data$year %nest% data$semester_fs)))


modeltest = lm(new_ach_gain ~1 + as.factor(quarter), data=data_box)
summary(modeltest)

# Categorizing $quarter
data_box$quarter[data_box$class.meet.weeks == 8] = "half"
data_box$quarter[data_box$class.meet.weeks == 10 | data_box$class.meet.weeks == 11] = "quarter"
data_box$quarter[data_box$class.meet.weeks == 13 | data_box$class.meet.weeks == 14 | data_box$class.meet.weeks == 15 | data_box$class.meet.weeks == 15.5 | data_box$class.meet.weeks == 16 | data_box$class.meet.weeks == 17 | data_box$class.meet.weeks == 18] = "semester"
table(data$institution, data$quarter)
#################################################################################################
...
model8 = lmer(new_ach_gain ~ 1   
              ...
              + department.classification
                  ...
                  + as.factor(time.meet.ind) # 1 = morning (7-10), 2 = afternoon (11-2), 3 = evening (after 2), 4 = ??
                  + as.factor(incentive.post.ind)
                  + as.factor(incentive.post.bin)
                  + as.factor(incentive.pre.ind)
                  + as.factor(incentive.pre.bin)
                  + admin.post.location.bin
                  + section_gpa
              ...
              + section_Overall_Attitude_Post
              + isi.workshop
              + as.factor(position.classification)
              + as.factor(advanced.stats.degree)
              + as.factor(student.type)
              + as.factor(analyzing.data.experience2)
              + as.factor(CarnegieClassification)
              + class.size.ind
                  + new_pre_perc*as.factor(gender) 
                  + instructor.gender2*as.factor(gender) 
                  + gpa_pre*as.factor(gender)
                  + gpa*as.factor(gender)
                  + SATACTzscore*as.factor(gender)
                  + Interest_pre*instructor.gender2
                  + Difficulty_pre*instructor.gender2
                  + Value_pre*instructor.gender2
                  + new_pre_perc*textbook
                  + new_pre_perc*gender
                  + new_pre_perc*instructor.gender2
                  + instructor.gender2*gender
                  + textbook*section_SATACTzscore
              + semester   
              + dep_ind3
              + ( 1 | institution/instructor.last.name/InstructorName_Section_Semester)
              , data=data_box, na.action = na.exclude, REML = FALSE )
summary(model8)
ranef(model8)
#REML convergence all years
#section (-80.5), instructor (-75.7), institution (-78.6)
#institution/instructor (-78.6), institution/section (-81.5), instructor/section (-80.5)
#institution/instructor/section (-81.5)
#1415 (no advanced.stats.degree, student.type)
#section (218.4), instructor (220.2), institution (221.7)
#institution/instructor (220.2), institution/section (218.2), instructor/section (217.2)
#institution/instructor/section (217.2)
#1516
#section (51.8), instructor (58.4), institution (57.9)
#institution/instructor (57.9), institution/section (51.8), instructor/section (51.8)
#institution/instructor/section (51.8)
#1617
#section (-), instructor (-), institution ()
#institution/instructor (), institution/section (-), instructor/section (-)
#institution/instructor/section ()
anova(model8)


### Start individual years model ####### drop any t < 3 and for factors F < 3 ############################################################
model1415 = lmer(new_ach_gain ~ 1   
                 + new_pre_perc #+ Affect_pre
                 + new_post_perc
               #  + Cognitive.Competence_pre #+ Difficulty_pre
               #  + Effort_pre
               ...
               #  + dep_ind3   
              + ( 1 | institution/instructor.last.name/InstructorName_Section_Semester)
              , data=clean1415, na.action = na.exclude, REML = FALSE )
summary(model1415)
anova(model1415)
ranef(model1415)

# Viewing random effects
randoms<-ranef(model1415, postVar = TRUE)
qq <- attr(ranef(model1415, postVar = TRUE)[[1]], "postVar")
rand.interc<-randoms$InstructorName_Section_Semester
df<-data.frame(Intercepts=randoms$InstructorName_Section_Semester[,1],
               sd.interc=2*sqrt(qq[,,1:length(qq)]),
               lev.names=rownames(rand.interc))
View(df)
# Viewing graph of random effects
#Graph
...
#This function takes all of the dredge output and filters it to only include your null model and those where the delta AIC is <= 10
 top_models <- my_dredge %>%
   tidyverse::filter(delta <= 10 | df ==  64 )
...
model1516 = lmer(new_ach_gain ~ 1   
               ...
               #  + dep_ind3   
              + ( 1 | institution/instructor.last.name/InstructorName_Section_Semester)
              , data=clean1516, na.action = na.exclude, REML = FALSE )
summary(model1516)
anova(model1516)
ranef(model1516)

randoms<-ranef(model1516, postVar = TRUE)
qq <- attr(ranef(model1516, postVar = TRUE)[[1]], "postVar")
rand.interc<-randoms$InstructorName_Section_Semester
df<-data.frame(Intercepts=randoms$InstructorName_Section_Semester[,1],
               sd.interc=2*sqrt(qq[,,1:length(qq)]),
               lev.names=rownames(rand.interc))
...
model1617 = lmer(new_ach_gain ~ 1   
               ...
               #  + Affect_change
                  #  + Effort_change
                   # model doesn't run with these 3 in it for 1617
                     # + Cog_change
                     # + Value_change
                     # + Difficulty_change
               #  + Interest_change
               ...
               #  + dep_ind3   
              + ( 1 | institution/instructor.last.name/InstructorName_Section_Semester)
              , data=clean1617, na.action = na.exclude, REML = FALSE )
summary(model1617)
anova(model1617)
ranef(model1617) #no ranef for 1617 for some reason
